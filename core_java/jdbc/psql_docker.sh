...
case $mode in
	...
	start)	
		...
		#Check if container jrvs-jdbc exists, if not, create it.
		if sudo docker ps | grep -q jrvs-jdbc; then
 		  echo "Container jrvs-jdbc already exists."
		else
		#Create a container using psql image with name=jrvs-psql
		sudo docker run --rm --name jrvs-jdbc -e POSTGRES_PASSWORD=$PGPASSWORD -d -v pgdata:/var/lib/postgresql/data -p 5432:5432 postgres
		echo "Container jrvs-jdbc is successfully created."
		fi
		...
		sudo docker container start jrvs-jdbc 
		echo "jrvs-jdbc container is running."
	...
	stop)	
		...
		#If exist, stop it.
		  ...
		  echo "jrvs-jdbc container is stopped."
 		  		
      		#If not exist, show error message and exit
                else
		  echo "Container jrvs-jdbc does not exist."
		  exit 1
		fi
		;;
        delete) 
